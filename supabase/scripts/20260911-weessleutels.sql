-- Weessleutels in cms_paginas
--
-- Na het opruimen van de 24 dode velden bleek de home-rij er 23 sleutels te
-- hebben terwijl het schema er 10 kent. Die dertien stonden nooit in
-- PAGE_FIELDS -- ze komen uit een ontwerp van vóór de velden die we gisteren
-- verwijderden. Ze doen niets: geen pagina leest ze en de editor toont ze niet,
-- want die genereert zijn velden uit het schema. Ze staan er alleen.
--
-- Dit soort drift is niet met een test te vangen: tests/unit/cms-pages.spec.ts
-- vergelijkt code met schema, en de database zit daar niet bij. Deze query is
-- de enige manier om het te zien; draai hem opnieuw na elke schemawijziging.
--
...
create temporary table schema_sleutels (slug text, sleutel text) on commit drop;

insert into schema_sleutels (slug, sleutel) values
...
-- 1 · Wat staat er te veel? Lees dit eerst.
select p.slug,
       k.key as weessleutel,
       left(p.data ->> k.key, 60) as waarde
  from cms_paginas p
  cross join lateral jsonb_object_keys(p.data) k(key)
 where not exists (
         select 1 from schema_sleutels s
          where s.slug = p.slug and s.sleutel = k.key)
 order by p.slug, k.key;

-- 2 · Pas hierna dit uit, als de lijst hierboven klopt.
...
--                    where s.slug = p.slug and s.sleutel = k.key));
--
-- -- Controle: elke pagina heeft precies zoveel velden als het schema kent.
-- select p.slug,
