...
--
-- Zelfde inhoud als supabase/migrations/20260914120000_admin-dashboard.sql;
-- deze kopie is om in de SQL Editor te plakken. Zonder deze functies laadt het
-- dashboard wel, maar blijven "Klaar om te publiceren" en de balk "Wat staat er
-- op de site" leeg (beide falen stil, met een regel in de Vercel-logs).
--
...
-- Controle: negen regels met per contenttype de live- en conceptaantallen,
-- en daaronder de concepten zelf.
select * from public.admin_status_aantallen() order by naam;
select soort, titel, bijgewerkt_op from public.admin_concepten(8);
