-- Weessleutels in cms_paginas -- stap 1 van 3: tonen
--
...
wees as (
  select p.slug, k.key, p.data -> k.key as waarde
    from cms_paginas p
    left join schema_sleutels s on s.slug = p.slug
    cross join lateral jsonb_object_keys(p.data) k(key)
   where not (k.key = any(coalesce(s.sleutels, array[]::text[])))
)
select slug, key as weessleutel, left(waarde #>> '{}', 60) as waarde
  from wees
 order by slug, key;
