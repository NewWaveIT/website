...
- **De admin is te bekijken zonder database: `/ontwerp`.** De admin zit achter
  Supabase-auth, CI komt daar niet in en lokaal staat geen werkende sleutel. Die route
  rendert de échte componenten met de rijen uit `lib/cms/voorbeeldrijen.ts`, zodat axe en
  een schermafdruk meten wat er werkelijk staat. Hij verving een met de hand geschreven
  kopie van de markup van 407 regels, die niets meer tegenhield zodra een component
...
- **Een adminscherm dat je wil kunnen tonen, scheidt ophalen van tekenen.**
  ...
  die je wil beoordelen; sindsdien rendert hij het echte scherm.
