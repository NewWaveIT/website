  ...
  return (
    <>
      ...
      <div className="card" style={{ padding: "var(--space-6)", maxWidth: 660 }}>
        <h3 style={{ fontSize: "var(--text-base)", fontWeight: "var(--fw-bold)", marginBottom: "var(--space-3)" }}>{titel}</h3>
        <div style={{ fontSize: "var(--text-sm)", color: "var(--text-body)", lineHeight: 1.7 }}>{children}</div>
      </div>
    </>
  );
}
...
export default async function GebruikersPage() {
    ...
    return (
      <Notice titel="Nog één stap nodig">
        <p style={{ margin: 0 }}>
          ...
          <Code>SUPABASE_SERVICE_ROLE_KEY</Code> in — lokaal in <Code>.env.local</Code> en in Vercel bij de
  ...
  if (fout) {
    const sleutelProbleem = /api key|invalid|jwt|unauthor|not allowed|permission|401|403/i.test(fout);
    return (
      <Notice titel={sleutelProbleem ? "De service_role-sleutel lijkt ongeldig" : "Kon gebruikers niet laden"}>
        {sleutelProbleem ? (
          <p style={{ margin: 0 }}>
            Supabase gaf terug: <Code>{fout}</Code>. Controleer of <Code>SUPABASE_SERVICE_ROLE_KEY</Code> exact de{" "}
            <strong>service_role</strong>-sleutel is (níet de <em>anon</em>-sleutel), volledig gekopieerd en zonder
            aanhalingstekens. Te vinden in Supabase onder <em>Project Settings → API → service_role</em>. Herstart de
            server na het wijzigen.
          </p>
        ) : (
          <p style={{ margin: 0 }}>Foutmelding: <Code>{fout}</Code></p>
        )}
      </Notice>
    );
  }
